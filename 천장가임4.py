...
def place_tianjiang(grid, start_zhi, tianpan_sequence):
    ...
    arranged_tianjiang = []
    for i in range(12):
        pos_index = (start_index + (-i if reverse_order else i)) % 12
        arranged_tianjiang.append(tianjiang_list[i])
    
    # 천장을 그리드에 배치
    for i, zhi in enumerate(tianpan_sequence):
        x, y = tianpan_coords[zhi]
        grid[x][y] = arranged_tianjiang[i]
...
def create_6x6_tianpan_and_tianjiang(month_zang_sequence, divination_hour, day_gan):
    ...
    # 천장 배열을 6x6 그리드에 배치
    place_tianjiang(combined_grid, tianjiang_start_zhi, tianpan_positions)
