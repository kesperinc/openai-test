...
def determine_noble(day_gan, hour_zhi):
    """
    천간과 점시를 기반으로 시작 귀인 위치를 결정하는 함수.
    """
    # 시간에 따른 귀인 위치 선택
    ...
    print("일간=",day_gan, "주야 귀인=", day_noble, night_noble)
...
def determine_start_noble_position(day_gan, hour_zhi):
    ...
    천간과 점시를 기반으로 시작 귀인 위치를 결정하는 함수.
    """
    start_noble_index, other_noble_index = determine_noble(day_gan, hour_zhi)
    
    return start_noble_index

def find_noble_start_index(day_gan, hour_zhi, month_zang_sequence):
    """
    천간과 점시를 기반으로 귀인의 시작 인덱스를 찾는 함수.
    """
    # 시작 귀인 위치 결정
    start_zhi = determine_start_noble_position(day_gan, hour_zhi)

    # 월장 리스트에서 시작 인덱스 찾기
    start_index = month_zang_sequence.index(start_zhi)
    print('월장=', month_zang_sequence)
    
    return start_index

def place_tianjiang(grid, start_zhi, tianpan_sequence):
    """
    천반에서 찾은 시작 지지를 기반으로 천장을 배치하는 함수.
    """
    # 천반의 위치 설정 (반시계 방향 순환)
    ...
    for i in range(12):
        ...
        tianjiang_sequence.append(tianjiang_list[i])
    ...
    print('천장 배열 =', tianjiang_sequence)

    print(tianjiang_sequence)

    # 천장을 그리드에 배치
    for i, zhi in enumerate(tianpan_sequence):
        x, y = tianpan_coords[zhi]
...
def create_6x6_tianpan_and_tianjiang(month_zang_sequence, divination_hour, day_gan, tianjiang_sequence):
    """
    ...
    # 천장의 시작 인덱스 찾기
    tianjiang_start_index = find_noble_start_index(day_gan, divination_hour, month_zang_sequence)

    # 천장의 시작 지지를 tianpan_positions에서 가져오기
    tianjiang_start_zhi = tianpan_positions[tianjiang_start_index]
    ...
    for i in range(12):
        ...
        combined_grid[x][y] = tianjiang_sequence[i]
...
def create_4x4_tianjiang(day_gan, divination_hour, tianjiang_sequence, month_zang_sequence):
    """
    ...
    # 천장의 시작 인덱스 찾기
    start_index = find_noble_start_index(day_gan, divination_hour, month_zang_sequence)

    # 점시에 따른 순행/역행 결정
    reverse_order = divination_hour in ['酉', '戌', '亥', '子', '丑', '寅']

    # 천장을 결합하여 4x4 배열에 배치
    if reverse_order:
        for i in range(12):
            pos_index = (start_index - i) % 12
            x, y = diban_coords[diban_positions[pos_index]]
            tianjiang_grid[x][y] = tianjiang_sequence[i]
    else:
        for i in range(12):
            pos_index = (start_index + i) % 12
            x, y = diban_coords[diban_positions[pos_index]]
...
def main():
    ...
    # 6x6 천반 및 천장 배열 초기화 
    tianjiang_sequence = tianjiang_list 
    create_6x6_tianpan_and_tianjiang(month_zang_sequence, hour_zhi, day_gan, tianjiang_sequence)

    # 4x4 천반 배열 출력
    create_4x4_tianpan(month_zang_sequence, hour_zhi)

    # 4x4 천장 배열 출력
    create_4x4_tianjiang(day_gan, hour_zhi, tianjiang_sequence, month_zang_sequence)
