...
def place_tianjiang(grid, tianjiang_sequence, positions, reverse=False):
    """
    천장 배열을 그리드에 배치하는 함수
    """
    # 천장의 순서에 따라 위치를 결정
    keys = list(positions.keys())
    for i, zhi in enumerate(keys):
        x, y = positions[zhi]
        if reverse:
            grid[x][y] = tianjiang_sequence[(12 - i) % 12]
        else:
...
def create_6x6_tianpan_and_tianjiang(month_zang_sequence, divination_hour, day_gan):
    ...
    place_tianjiang(combined_grid, tianjiang_sequence, tianjiang_positions, reverse=reverse_order)
...
def create_4x4_tianjiang(day_gan, divination_hour, month_zang_sequence):
    ...
    # 천장을 결합하여 4x4 배열에 배치
    place_tianjiang(tianjiang_grid, tianjiang_sequence, diban_coords, reverse=reverse_order)
