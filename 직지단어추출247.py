...
# 처리할 파일들의 리스트를 가져옴 (normalized_files 폴더 내의 파일을 처리)
normalized_dir = 'normalized_files/'
file_list = [normalized_dir + f for f in os.listdir(normalized_dir) if f.startswith('normalized_')]
...
with open('jikji_5char.txt', 'w', encoding='utf-8') as result_5char_file:
    # 한자만 저장
...
print("모든 파일에서 중복되지 않는 5글자 이상의 명사는 jikji_5char.txt에 저장되었습니다.")
